...
class BulkLoader {
  ...
  static Future<void> loadAll(String user) async {
    ...
    status.value = '';
    Store.tick.value++;
    // ✅ إبطال cache السلاسل بعد التحميل
...
/* ============================================================
   ✅ نظام Cache + Isolate لتجميع السلاسل (إصلاح مشكلة التعليق)
   ============================================================ */

/// Cache للتجميع: يتجنب إعادة الحساب في كل build
List<Movie>? _seriesCacheResult;
int _seriesCacheTick = -1;
String _seriesCacheScope = '';
bool _seriesComputing = false;

/// إبطال الكاش عند أي تغيير في البيانات
void invalidateSeriesCache() {
  _seriesCacheTick = -1;
  _seriesCacheResult = null;
}

/// ✅ التجميع الصارم النهائي (نسخة متزامنة - سريعة للـ cache)
List<Movie> groupMoviesSmart(List<Movie> all) {
  // التحقق من الكاش أولاً
  final currentScope = App.scope.value;
  if (_seriesCacheResult != null && 
      _seriesCacheTick == Store.tick.value && 
      _seriesCacheScope == currentScope) {
    return _seriesCacheResult!;
  }
  ...
  // معالجة السلاسل المكتشفة
  for (final e in groups.entries) {
    if (e.value.length >= 2) {
      final list = List<Movie>.from(e.value)
        ..sort((a, b) => (a.year == b.year) 
            ? a.msgId.compareTo(b.msgId) 
...
/// ✅ نسخة غير متزامنة (تشغّل في Isolate) للاستدعاء الأول
Future<List<Movie>> groupMoviesSmartAsync(List<Movie> all) async {
  if (_seriesComputing) {
    // انتظر حتى ينتهي الحساب الحالي
    while (_seriesComputing) {
      await Future.delayed(const Duration(milliseconds: 50));
    }
    return _seriesCacheResult ?? all;
  }
  
  _seriesComputing = true;
  try {
    // ✅ تشغيل في Isolate منفصل لعدم تجميد الـ UI
    final result = await compute(_groupMoviesIsolate, all);
    
    // إعادة بناء SeriesRegistry على الـ main thread
    // (لأن الـ Isolate لا يشارك الذاكرة)
    SeriesRegistry.parts.clear();
    SeriesRegistry.names.clear();
    ...
    _seriesCacheResult = result;
    _seriesCacheTick = Store.tick.value;
    _seriesCacheScope = App.scope.value;
    
    return result;
  } finally {
    _seriesComputing = false;
  }
}

/// الدالة التي تُنفّذ في الـ Isolate
List<Movie> _groupMoviesIsolate(List<Movie> all) {
  final deduped = Smart.dedup(all);
  final groups = <String, List<Movie>>{};
  final result = <Movie>[];
...
/// ✅ حساب مسبق يُستدعى من initState
Future<void> precomputeSeries() async {
  if (_seriesCacheResult != null && _seriesCacheTick == Store.tick.value) {
    return; // الكاش صالح
  }
  final all = Store.all();
  if (all.isEmpty) return;
  await groupMoviesSmartAsync(all);
}
