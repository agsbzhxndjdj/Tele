...
class _HomePageState extends State<HomePage> {
  ...
  void _tick() {
    // ✅ تحسين: Debounce لمنع إعادة البناء المتكررة
    _debounce?.cancel();
  ...
  Widget build(BuildContext context) {
    ...
    final genres = <String>{};
    // ✅ تحسين: أخذ أول 50 فيلم فقط لاستخراج الأنواع
        ...
        SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          ? PreferredSize(preferredSize: const Size.fromHeight(56),
              child: Padding(padding: const EdgeInsets.fromLTRB(12, 0, 12, 10),
                  child: TextField(controller: _search, onChanged: (_) { _invalidateCache(); setState(() {}); },
  ...
  slivers: [
                  ...
                  if (movies.isEmpty)
                    ...
                    SliverList(delegate: SliverChildBuilderDelegate((_, i) => MovieRowItem(m: movies[i]), childCount: movies.length))
                  else
                    SliverGrid(
...
class MovieCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
      // ✅ إصلاح: إزالة Store.tick.value من الـ Key
      key: ValueKey('card_${m.id}'),
      clipBehavior: Clip.antiAlias, margin: const EdgeInsets.all(6),
      child: InkWell(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SeriesRegistry.isSeries(m.id) ? SeriesPartsScreen(m: m) : MovieDetailsScreen(m: m))),
          child: Stack(fit: StackFit.expand, children: [
            ...
            Positioned(top: 48, left: 4, child: Row(mainAxisSize: MainAxisSize.min, children: [
              // ✅ إصلاح: استخدام ValueListenableBuilder فقط عند الحاجة
