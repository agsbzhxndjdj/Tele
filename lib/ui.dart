...
class _HomePageState extends State<HomePage> {
  ...
  List<Movie> _cachedBase = [];
  int _cachedTick = -1;
  List<Movie> _cachedCont = [];
  Timer? _debounce;
...
void dispose() {
  ...
  List<Movie> get _base {
    if (_cachedTick == Store.tick.value && _cachedBase.isNotEmpty) return _cachedBase;
  ...
  List<Movie> get _continueWatching {
    if (_cachedTick == Store.tick.value && _cachedCont.isNotEmpty) return _cachedCont;
    _cachedCont = Store.all().where(_inProgress).toList();
    return _cachedCont;
  }
  ...
  Widget build(BuildContext context) {
    final movies = _base;
    final cont = _continueWatching;
    final genres = <String>{};
    for (final m in movies.take(50)) genres.addAll(m.genres.take(3));
                      ...
                      slivers: [
                        ...
                        if (movies.isEmpty)
                          SliverToBoxAdapter(
                            child: SizedBox(
                              height: 200,
                              child: Center(child: Text(Lang.t('noMovies'), style: const TextStyle(color: Colors.grey))),
                            ),
                          )
                        else if (listView)
                          SliverList(
                            delegate: SliverChildBuilderDelegate(
                              (_, i) => MovieRowItem(m: movies[i]),
          ...
          children: [
            SizedBox(
              ...
              child: ListView(
                ...
                children: [
                  const SizedBox(width: 10),
                  _chip(Lang.t('all'), _fq == '', () => setState(() { _fq = ''; _invalidateCache(); })),
                  ...['1080P', '720P', '480P'].map((q) => _chip(q, _fq == q, () => setState(() { _fq = q; _invalidateCache(); }))),
                  const SizedBox(width: 10),
                ],
              ),
            ),
            if (genres.isNotEmpty)
              SizedBox(
                ...
                child: ListView(
                  ...
                  children: [
                    const SizedBox(width: 10),
                    _chip(Lang.t('all'), _fg == '', () => setState(() { _fg = ''; _invalidateCache(); })),
                    ...genres.take(12).map((g) => _chip(g, _fg == g, () => setState(() { _fg = g; _invalidateCache(); }))),
                    const SizedBox(width: 10),
                  ],
                ),
              ),
            ...
            children: [
              m.poster.isNotEmpty
                  ? (Store.getBool('heroFx', true)
                      ? Hero(
                          tag: 'poster_${m.id}',
                          child: CachedNetworkImage(imageUrl: m.poster, fit: BoxFit.cover, placeholder: (_, __) => _ph(), errorWidget: (_, __, ___) => _ph()),
                        )
                  ...
                  children: [
                    ValueListenableBuilder<int>(
                      ...
                      builder: (_, __, ___) => IconButton(
                        icon: Icon(
                          ...
                          size: 20,
                          color: Store.isFav(m.id) ? Colors.red : Colors.white70,
                        ),
                    ...
                    ValueListenableBuilder<Map<String, double>>(
                      ...
                      builder: (_, prog, __) => prog.containsKey(m.id)
                          ...
                          : IconButton(
                              icon: const Icon(Icons.download_for_offline, size: 20, color: Colors.white70),
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(m.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
                    ),
                    if (m.qualityOptions.length > 1)
                    ...
                    ValueListenableBuilder<int>(
                      ...
                      builder: (_, __, ___) => IconButton(
                        icon: Icon(
                          Store.isLater(m.id) ? Icons.bookmark : Icons.bookmark_border,
                          color: Store.isLater(m.id) ? AppTheme.accent : Colors.white70,
                        ),
                    ...
                    children: [
                        ...
                        builder: (_, __, ___) => Row(
                          children: [
                            ...List.generate(
                              ...
                              (i) => IconButton(
                                icon: Icon(
                                  i < (Store.ratings()[m.id] ?? 0) ? Icons.star : Icons.star_border,
                                  size: 22,
                            ...
                            IconButton(
                              ...
                              onPressed: () => showPlaylistDialog(context, m),
                            ),
                            IconButton(
                              icon: const Icon(Icons.comment_outlined, color: Colors.white70),
                              onPressed: () => CommentsSheet.show(context, m),
                            ),
                            IconButton(
                              icon: const Icon(Icons.music_note_outlined, color: Colors.white70),
                              onPressed: () => Navigator.push(
                                context,
                                MaterialPageRoute(builder: (_) => TrailerScreen(query: '${m.title} OST soundtrack')),
                              ),
                            ),
                            IconButton(
                              icon: const Icon(Icons.share_outlined, color: Colors.white70),
                              onPressed: () => ShareCard.show(context, m),
                            ),
                            IconButton(
                              icon: Icon(
                                Store.isPinned(m.id) ? Icons.push_pin : Icons.push_pin_outlined,
                                color: Store.isPinned(m.id) ? AppTheme.accent : Colors.white70,
                              ),
                              onPressed: () => Store.togglePin(m.id),
                            ),
                            IconButton(
                              icon: const Icon(Icons.local_fire_department_outlined, color: Colors.white70),
                              onPressed: () => Marathon.dialog(context, m),
                            ),
                            IconButton(
                              icon: const Icon(Icons.visibility_off_outlined, color: Colors.white70),
                              onPressed: () => Vault.quickHide(context, m),
                            ),
                            IconButton(
                              icon: const Icon(Icons.cast_outlined, color: Colors.white70),
                              onPressed: () => CastTv.open(context, m),
                            ),
                          ],
                        ),
                      ),
                      ...
                      if (ov.isNotEmpty) ...[
                        ...
                        ConstrainedBox(
                          ...
                          child: SingleChildScrollView(
                            child: Text(ov, style: TextStyle(color: Colors.grey.shade300, fontSize: 13, height: 1.6)),
                          ),
                        ),
                      ],
                      if (_facts.isNotEmpty) ...[
                        ...
                        Wrap(
                          spacing: 6,
                          runSpacing: 6,
                          children: _facts.map((f) => _chip('💡 $f')).toList(),
                        ),
                      ],
                      if (_cast.isNotEmpty) ...[
                        const SizedBox(height: 12),
                                  ...
                                  (c) => Padding(
                                    ...
                                    child: Column(
                                      children: [
                                        CircleAvatar(
                                          child: Text(
                                            (c['name'] ?? '؟').toString().split(' ').where((e) => e.isNotEmpty).map((e) => e[0]).take(2).join(),
                                          ),
                                        ),
                      ...
                      if (SeriesRegistry.isSeries(m.id)) ...[
                        ...
                        Wrap(
                          spacing: 6,
                          runSpacing: 6,
                          children: SeriesRegistry.partsOf(m.id)
                              .asMap()
                              .entries
                              .map(
                                (e) => ActionChip(
                                  label: Text('الجزء ${e.key + 1}', style: const TextStyle(fontSize: 10)),
                                  onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MovieDetailsScreen(m: e.value))),
                                ),
                              )
                              .toList(),
                        ),
                      ...
                      if (Parts.partsOf(m).isNotEmpty) ...[
                        ...
                        Wrap(
                          spacing: 6,
                          children: Parts.partsOf(m)
                              .map(
                                (p) => ActionChip(
                                  label: Text(p.title, style: const TextStyle(fontSize: 10)),
                                  onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MovieDetailsScreen(m: p))),
                                ),
                              )
                              .toList(),
                        ),
                        ...
                        children: [
                          Expanded(
                            child: FilledButton.icon(
                              onPressed: () => Navigator.push(
                                context,
                                MaterialPageRoute(builder: (_) => PlayerScreen(title: m.title, url: m.videoUrl, movie: m)),
                              ),
          ...
          FilledButton(
            onPressed: () {
              ...
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(builder: (_) => PlayerScreen(title: widget.next!.title, url: widget.next!.videoUrl, movie: widget.next)),
              );
            },
        ...
        children: [
          ...
          GestureDetector(
            ...
            onHorizontalDragUpdate: (d) {
              ...
              setState(() {
                _gmode = 1;
                _glabel = '${_seekDelta >= 0 ? '+' : ''}$_seekDelta → ${_fmt(Duration(seconds: (_seekBase + _seekDelta).clamp(0, dur.inSeconds)))}';
              });
            },
            ...
            Center(
              child: Glass(
                ...
                child: Column(
                  ...
                  children: [
                    Icon(
                      _gmode == 2 ? Icons.volume_up : _gmode == 3 ? Icons.brightness_6 : Icons.fast_forward,
                      color: AppTheme.accent,
                      size: 34,
                    ),
          ...
          if (_locked && _ui)
            Center(
              child: IconButton(
                icon: const Icon(Icons.lock_open, size: 40, color: Colors.white70),
                onPressed: () => setState(() {
                  _locked = false;
                  _ui = false;
                }),
                ...
                child: Glass(
                  ...
                  child: Row(
                    children: [
                      IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white), onPressed: () => Navigator.pop(context)),
                      Expanded(
                        child: Text(widget.title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
                      ),
                      ...
                      IconButton(
                        icon: Icon(_locked ? Icons.lock : Icons.lock_open, color: Colors.white70),
                        onPressed: () => setState(() {
                          _locked = !_locked;
                          _ui = false;
                        }),
                      ...
                      IconButton(
                        icon: Icon(_audioOnly ? Icons.videocam : Icons.music_note, color: Colors.white70),
                        onPressed: () {
                      ...
                      IconButton(
                        icon: Icon(_isLandscape ? Icons.stay_current_portrait : Icons.stay_current_landscape, color: Colors.white70),
                        onPressed: () {
                          setState(() => _isLandscape = !_isLandscape);
                          SystemChrome.setPreferredOrientations(
                            _isLandscape ? [DeviceOrientation.landscapeLeft, DeviceOrientation.landscapeRight] : [DeviceOrientation.portraitUp],
                          );
                        },
                      ),
                    ],
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () async {
                        ...
                        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(r)));
                      },
